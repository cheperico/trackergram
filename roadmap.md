...
| **F4-7** | **Cache local de messageIds para deduplicación sin API** | 2 sesiones | ✅ **v0.7.1 (BUG-009)** — `TikiWikiClient` implementó cache local `tmp/message_ids_{trackerId}.json` mapeando `(chatId, messageId) → itemId`, sembrado UNA vez desde `getAllTrackerItems()` (fetch FUERA del lock, code review M1) y actualizado en `createTrackerItem()`. `messageExists()`/`findItemByMessageId()` usan el cache (O(1), sin HTTP call por mensaje). Poda a 1000 si supera 5000, LOCK_EX/LOCK_SH. `getAllTrackerItems()` devuelve null en error y el cache NO se escribe si falla. `invalidateMessageIdsCache()` evita cache stale cuando items se crean fuera del webhook (import.php lo llama, code review M2). **Pendiente**: GC probabilístico de cache files viejos (como rate files) y re-seed periódico para items creados manualmente fuera de trackerGram. |
